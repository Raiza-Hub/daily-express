...
- Job layer:
  - `workers/boss.ts`: queues are `TRIP_REFUND` (+ DLQ) and `EMAIL_SEND` (+ DLQ). Payout initiation is **not** job-based — it runs synchronously post-commit in `trip.service.ts` `completeTrip`.
  - `workers/job.service.ts`: `enqueueTripRefund` (trip.refund) + `enqueueEmail` (email.send). `enqueuePayout` removed.
  - `workers/trip-refund.worker.ts` + `workers/email.worker.ts`: only registered consumers (`workers/index.ts` `registerTripRefundWorker`/`registerEmailWorker`). `workers/payout.worker.ts` deleted.
