...
- `payout/earning.service.ts` `completeTrip` (`markTripCompletedInTransaction`): releases pending → available only. No enqueue, no `hasUnsettledEarnings` branch (retained solely for re-completion gating in `trip.service.ts`).
...
- `route/trip.service.ts` `completeTrip`: allows re-completion iff `hasUnsettledEarnings(tripId)` (otherwise still throws "Trip is already completed"). After the completion tx commits, calls `payoutService.triggerPayout(tripId)` synchronously (post-commit, outside any tx) with `try/catch` + `logger.error("payout.trigger_after_complete.failed")` — the driver awaits the request, so payout initiation is not job-based. `getDailyTripSummaries` returns per-trip latest `payoutStatus` via `selectDistinctOn(payout.tripId)` ordered by `createdAt DESC, id DESC`.
- Frontend:
