...
- `payout/payout-settlement.service.ts` (NEW, replaces `payout-attempt.service.ts`): `verifyWithProvider(payout)` (lookup by `payout.reference`; no record → `"failed"`, success → `finalizePayout`, provider failed → `"failed"`, else `"processing"`, lookup API error → `"unknown"`); `finalizePayout(payout, raw)` (immutability guard `success`/`failed` → skip; payout → success, trip earnings → paid, stats adjusted, success notification).
