...
- `workers/payer-info.worker.ts` (NEW): consumes `payment.payer-info` (retry 3×/backoff → DLQ log-drop). `verifyTransaction(reference)`; non-`success` or no `payer_bank_account` → silent `return null` (signed webhook trusted); else tx-updates `payment.payerBankName`/`payerAccountNumber`/`payerAccountName` where `status = 'successful'` (idempotent; refunds read these fields from `payment`). Migration `0032_drop_payment_failure_fields.sql` (NOT APPLIED) drops `payment.failed_at`/`failure_code`/`failure_reason`; `refund.failure_reason` retained.
...
- `dailyexpress-api/db/payment-schema.ts` — dropped `failedAt`/`failureCode`/`failureReason` from `payment` (`refund.failure_reason` retained).
