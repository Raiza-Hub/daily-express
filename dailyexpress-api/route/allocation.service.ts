    ...
    const result = await db.transaction(async (tx) => {
      ...
      await tx.execute(sql`
        SELECT pg_advisory_xact_lock(
          hashtext(concat(${bookingRecord.routeId}::text, ${tripDateStr}::text, ${bookingRecord.vehicleType}::text))::bigint
        )
      `);
      ...
      if (bestTripRow.length > 0) {
        ...
        const newTripRows = (await tx.execute(sql`
          ...
          VALUES (${bookingRecord.routeId}, ${tripDateStr}, ${bookingRecord.vehicleType}, ${capacity}, 0, 'awaiting_driver', now(), now())
