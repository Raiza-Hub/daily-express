...
export const PUBLIC_PATHS: readonly PublicPath[] = [
  ...
  {
    pattern: /^\/api\/v1\/route\/search$/,
    methods: ["GET"],
    description: "Search routes by criteria",
    group: "route",
  },
  {
    pattern: /^\/api\/v1\/payments\/webhooks\/kora$/,
    methods: ["POST"],
    description: "Kora payment webhook",
    group: "payment",
  },
  {
    pattern: /^\/api\/v1\/payments\/return$/,
    methods: ["GET"],
    description: "Hosted checkout return",
    group: "payment",
  },
  {
    pattern: /^\/api\/v1\/payouts\/webhooks\/kora$/,
    methods: ["POST"],
...
function matchesPublicPath(
  ...
  return PUBLIC_PATHS.some((publicPath) => {
    ...
    return (
      publicPath.methods.includes(method) &&
...
export function isWebhookPath(path: string, method: string): boolean {
  const pathname = getPathname(path);

  return PUBLIC_PATHS.some((publicPath) => {
    return (
      (publicPath.group === "payment" || publicPath.group === "payout") &&
      publicPath.description.toLowerCase().includes("webhook") &&
      publicPath.methods.includes(method) &&
      publicPath.pattern.test(pathname)
    );
  });
